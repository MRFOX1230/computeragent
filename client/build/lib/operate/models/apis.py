...
def call_gemini_pro_with_ocr(messages, objective, model):
    ...
    try:
        ...
        try:
            ...
            print(f"{ANSI_RED}GEMINI ERROR: Model did not return valid JSON. Response was: {content}{ANSI_RESET}")
        ...
        print(f"{ANSI_RED}GEMINI API ERROR: {e}{ANSI_RESET}")
        if config.verbose:
            traceback.print_exc()
...
def call_ollama_llava_with_ocr(messages, objective, model):
    ...
    try:
        ...
        try:
            ...
            print(f"{ANSI_RED}[Ollama] ERROR: Model did not return valid JSON. Response was: {content}{ANSI_RESET}")
        ...
        print(f"{ANSI_RED}[Ollama] ERROR: Couldn't connect to Ollama. Run `ollama serve`.{ANSI_RESET}\nDetails: {e}")
        return [] # Возвращаем пустой список при ошибке

    except Exception as e:
        print(f"{ANSI_RED}[Ollama] ERROR: An unexpected error occurred: {e}{ANSI_RESET}")
        if config.verbose:
            traceback.print_exc()
