...
class Config:
    ...
    def validation(self, model, voice_mode):
        ...
        self.require_api_key(
            "GOOGLE_API_KEY", "Google API key", model == "gemini-pro-with-ocr"
        )
        self.require_api_key(
    ...
    def require_api_key(self, key_name, key_description, is_required):
        ...
        if self.verbose:
            print(f"[Config] Checking for required key: {key_name}")
            
        if is_required and not key_exists:
            # Новая логика: выводим ошибку и принудительно завершаем программу
            print(f"ОШИБКА: Необходимый API-ключ '{key_name}' не найден.")
            print("Пожалуйста, добавьте его в ваш файл .env или установите как переменную окружения.")
